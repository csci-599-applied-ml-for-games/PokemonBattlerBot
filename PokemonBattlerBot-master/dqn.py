...
class DQNAgent():
	...
	def train(self, terminal_state):
		...
		self.model.fit(np.array(X), np.array(y), batch_size=MINIBATCH_SIZE, 
			verbose=0, shuffle=False,callbacks=[history])
		self.loss.append(history.losses[0])
		self.log(f'losses ********: {self.loss}')
		if terminal_state:
	...
	def draw_plot(self):
		...
		plt.title("Plot of the loss value")
		plt.xlabel("number of batch")
		plt.ylabel("loss value")
		plt.plot(list(range(len(self.loss))), self.loss, "b-o")
		plt.show()