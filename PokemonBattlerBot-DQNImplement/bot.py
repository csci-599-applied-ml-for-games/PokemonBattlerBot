...
class BotClient(showdown.Client):
	...
	async def switch_pokemon2(self, room_obj, data):
		team_info = self.get_team_info(data)
		valid_actions = []
		for pokemon_index, pokemon_info in enumerate(team_info):
	...
	async def take_start_index(self,room_obj, team):
		valid_actions = []
		for pokemon_index, pokemon_name in enumerate(team):
			valid_actions.append((pokemon_index + 1,
								  pokemon_name,
								  ActionType.Switch))
		...
		if room_obj and room_obj.id.startswith('battle-'):
			if inp_type == 'poke':
				...
				if (len(self.team) == self.teamsize and
					...
					self.take_start_index(room_obj,self.team)
				...
				if force_switch == True:
					...
					await self.switch_pokemon2(room_obj, data)
				else:
