		...
		if room_obj and room_obj.id.startswith('battle-'):
			if inp_type == 'poke':
				...
				for pokemon_info in team_info:
					...
					self.team_stats[str(pokemon_info['details'].rstrip(', M').rstrip(', F'))] = pokemon_info['stats']
				...
				self.log('team stats', self.team_stats)	
			...
			## track player1 and player2  buff
			elif inp_type == '-unboost':
				if ('p1a' in str(params[0])):
					# player 1  pokemon
					pokemon = params[0].strip('p1a: ')
					self.enemy_state.update_buff_list(pokemon, params[1],-1*params[2])
					self.log('P1 pokemon: ', pokemon)
					self.log('P1 unboost type: ', params[1])
					self.log('P1 unboost level:', params[2])
				else:
					# player 2 pokemon
					pokemon = params[0].strip('p2a: ')
					self.log('P2 pokemon: ', pokemon)
					self.log('P2 unboost type: ', params[1])
					self.log('P2 unboost level:', params[2])
			elif inp_type == '-boost':
				if ('p1a' in str(params[0])):
					# player 1  pokemon
					pokemon = params[0].strip('p1a: ')
					self.enemy_state.update_buff_list(pokemon, params[1],params[2])
					self.log('P1 pokemon: ', pokemon)
					self.log('P1 boost type: ', params[1])
					self.log('P1 boost level:', params[2])
				else:
					# player 2 pokemon
					pokemon = params[0].strip('p2a: ')
					self.log('P2 pokemon: ', pokemon)
					self.log('P2 boost type: ', params[1])
					self.log('P2 boost level:', params[2])
...
class EnemyState():
	...
	def __init__(self, opposing_team):
		...
		self.pokemon_items = {}
		self.team_stats = {} # Pokemon stats
		self.team_buff = {} # Pokemon buff
		self.buff_level = {};
		# add future states
	...
	def update_stats(self, pokemon, stats):
		self.team_stats[str(pokemon)] = stats
	def __create_bufflist(self):
		for pokemon in self.team_status:
			self.team_buff[pokemon] = []
			self.buff_level[pokemon]=0;
	def update_buff_list(self, pokemon, buff,level):
		self.buff_level[pokemon] += level
		if buff not in self.team_buff[pokemon]:
			self.team_buff[pokemon].append(buff)
	# end update_abilities
